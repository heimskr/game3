...
namespace Game3 {
	...
	int march8(const Position &position, const std::function<bool(int8_t, int8_t)> &get) {
	...
	int march4(const Position &position, const std::function<bool(int8_t, int8_t)> &get) {
		const bool center = get(0, 0);
		int sum = 0;
		if (center) {
			const auto top    = static_cast<uint8_t>(get(position.column,     position.row - 1));
