	...
	void Game::loadData(const nlohmann::json &json) {
		...
		if (type == "base:entity_texture_map") {
			...
			if (getSide() == Side::Client) {
				for (const auto &[key, value]: json.at(1).items()) {
					if (value.size() == 1) {
						textures.add(Identifier(key), std::make_shared<Texture>(Identifier(key), value.at(0).get<std::filesystem::path>()))->init();
					} else if (value.size() == 2) {
						textures.add(Identifier(key), std::make_shared<Texture>(Identifier(key), value.at(0).get<std::filesystem::path>(), value.at(1)))->init();
					} else if (value.size() == 3) {
						textures.add(Identifier(key), std::make_shared<Texture>(Identifier(key), value.at(0).get<std::filesystem::path>(), value.at(1), value.at(2)))->init();
					} else {
			...
			for (const auto &[key, value]: json.at(1).items()) {
				exemplars.add(Identifier(key), RegisterableIdentifier(Identifier(key), value.get<Identifier>()));
			}
