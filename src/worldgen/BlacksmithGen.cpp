...
namespace Game3::WorldGen {
	void generateBlacksmith(const std::shared_ptr<Realm> &realm, std::default_random_engine &rng, const std::shared_ptr<Realm> &parent_realm, Index width, Index height, const Position &entrance) {
		...
		realm->setTile(2, {1, 3}, "base:tile/furnace"_id, false, true);
		realm->setTile(2, {1, 5}, "base:tile/anvil"_id, false, true);
		...
		realm->setTile(2, {height / 2, width / 2 - 1}, "base:tile/counter_w"_id, false, true);
		realm->setTile(2, {height / 2, width / 2},     "base:tile/counter_we"_id, false, true);
		realm->setTile(2, {height / 2, width / 2 + 1}, "base:tile/counter_e"_id, false, true);
