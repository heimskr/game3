...
namespace Game3 {
	Plantable::Plantable(ItemID identifier_, std::string name_, Identifier tilename_, MoneyCount base_price, ItemCount max_count):
	...
	bool Plantable::use(Slot slot, ItemStack &stack, const Place &place) {
		...
		if (realm.pathMap[realm.getIndex(position)] && realm.getTileset().getEmptyID() == realm.getLayer2(position)) {
