...
namespace Game3 {
	...
	void Mushroom::getOffsets(const Game &game, std::shared_ptr<Texture> &texture, float &x_offset, float &y_offset) {
		...
		texture->init();
		x_offset = static_cast<float>(subID % 6) * 8.f;
	...
	Glib::RefPtr<Gdk::Pixbuf> Mushroom::makeImage(const Game &game) {
		...
		const size_t row_size = static_cast<size_t>(channels) * width;

		const auto x = (subID % 6) * 16;
		const auto y = (subID / 6) * 16;

		if (!rawImage) {
			rawImage = std::make_unique<uint8_t[]>(static_cast<size_t>(channels) * width * height);
			uint8_t *raw_pointer = rawImage.get();
			uint8_t *texture_pointer = texture->data->get() + static_cast<ptrdiff_t>(y) * *texture->width * channels + static_cast<ptrdiff_t>(x) * channels;
