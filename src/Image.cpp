...
namespace Game3 {
	...
	void Image::draw(float x, float y, float angle, float alpha) {
		if (nvg_ != -1)
			draw(x, y, 0, 0, width_, height_, angle, alpha);
	}

	void Image::draw(float x, float y, float x_offset, float y_offset, float size_x, float size_y, float angle, float alpha) {
		if (nvg_ == -1)
			return;
		nvgSave(context_);
		auto pattern = nvgImagePattern(context_, x + x_offset, y + y_offset, width_, height_, angle, nvg_, alpha);
		nvgBeginPath(context_);
		if (size_x < 0)
			size_x = width_;
		if (size_y < 0)
			size_y = height_;
		nvgRect(context_, x, y, size_x, size_y);
